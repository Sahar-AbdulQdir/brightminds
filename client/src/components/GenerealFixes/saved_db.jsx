...
const SavedPage = () => {
  ...
  const loadSavedItems = () => {
    ...
    const episodesWithType = savedEpisodes.map(item => ({
      ...item,
      type: "podcast",
      id: item.id || `podcast-${Math.random()}`
    }));

    const booksWithType = savedBooks.map(item => ({
      ...item,
      type: "book",
      id: item.identifier,
      title: item.title,
    ...
    setSavedItems([...episodesWithType, ...booksWithType]);
  };
  ...
  const removeItem = id => {
    ...
    if (itemToRemove?.type === "podcast") {
      const savedEpisodes =
        JSON.parse(localStorage.getItem("savedEpisodes")) || [];
      localStorage.setItem(
        "savedEpisodes",
        JSON.stringify(savedEpisodes.filter(ep => ep.id !== id))
      );
    } else {
      const savedBooks =
        JSON.parse(localStorage.getItem("savedBooks")) || [];
      localStorage.setItem(
        "savedBooks",
        JSON.stringify(
          savedBooks.filter(book => book.identifier !== id)
        )
      );
    }
  };
      ...
      <div className="saved-page-container">
        ...
        <div className="saved-header">
          ...
          <div className="filter-buttons">
            ...
            >
              <FaPlay /> Podcasts (
              {savedItems.filter(i => i.type === "podcast").length})
            </button>
            ...
            >
              <FaBookmark /> Books (
              {savedItems.filter(i => i.type === "book").length})
            </button>
          </div>
        </div>
